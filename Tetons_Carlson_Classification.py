...
lake_group = lake_group.dropna(subset=['mean_carlson', 'mean_vollen'])
...
plt.axvspan(20, 26, color='#0051e9', alpha=0.5, label='Strongly Oligotrophic (Carlson)')  # Light Blue
plt.axvspan(26, 33, color='#1d9df2', alpha=0.5, label='Oligotrophic (Carlson)')  # Distinct Blue
plt.axvspan(33, 38, color='#5dade2', alpha=0.5, label='Slightly Oligotrophic (Carlson)')  # Distinct Blue
 
# Mesotrophic category (Green shades, Light to Dark)
plt.axvspan(38, 43, color='#d5f5e3', alpha=0.5, label='Slightly Mesotrophic (Carlson)')  # Light Green
plt.axvspan(43, 49, color='#82e0aa', alpha=0.5, label='Mesotrophic (Carlson)')  # Mid Green
plt.axvspan(49, 54, color='#28b463', alpha=0.5, label='Strongly Mesotrophic (Carlson)')  # Dark Green
 
# Eutrophic category (Yellow shades, Light to Dark)
plt.axvspan(54, 58, color='#f9e79f', alpha=0.5, label='Slightly Eutrophic (Carlson)')  # Light Yellow
plt.axvspan(58, 62, color='#f5b041', alpha=0.5, label='Eutrophic (Carlson)')  # Mid Yellow
plt.axvspan(62, 65, color='#d35400', alpha=0.5, label='Strongly Eutrophic (Carlson)')  # Dark Orange
 
# Slightly Hypereutrophic category (Red shades)
plt.axvspan(65, 70, color='#c0392b', alpha=0.5, label='Slightly Hypereutrophic (Carlson)')  # Dark Red
